...
RUN mkdir -p /opt && git clone https://bitbucket.org/padavan/rt-n56u.git /opt/rt-n56u
RUN cd /opt/rt-n56u/toolchain-mipsel && ./clean_sources && ./build_toolchain
RUN cd /opt/rt-n56u/trunk && ./clear_tree


# fix error, see: https://bitbucket.org/padavan/rt-n56u/issues/809/build-error-on-fedora-26-and-patch-for-fix
COPY fix.patch /opt/fix.patch
RUN cd /opt/rt-n56u && git apply ../fix.patch
